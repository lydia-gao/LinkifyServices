from celery import Celery, current_app as current_celery_app
from celery.result import AsyncResult

from app.core.celery_config import settings as celery_settings


def create_celery() -> Celery:
	"""Create/configure a Celery app using settings in core.celery_config."""
	celery_app = current_celery_app
	celery_app.config_from_object(celery_settings, namespace="CELERY")
	# sensible defaults
	celery_app.conf.update(task_track_started=True)
	celery_app.conf.update(task_serializer="pickle")
	celery_app.conf.update(result_serializer="pickle")
	celery_app.conf.update(accept_content=["pickle", "json"])
	celery_app.conf.update(result_persistent=True)
	celery_app.conf.update(worker_send_task_events=False)
	celery_app.conf.update(worker_prefetch_multiplier=1)
	return celery_app


def get_task_info(task_id: str) -> dict:
	"""Return task status and result for a given task_id."""
	task_result = AsyncResult(task_id)
	return {
		"task_id": task_id,
		"task_status": task_result.status,
		"task_result": task_result.result,
	}


# Expose a module-level Celery instance so we can run:
# celery -A app.celery_app.celery worker ...
celery = create_celery()

